...
def find_path(image, tuning_params):
    ...
    params.filterByCircularity = False  # Disable circularity filter

    # Filter by Convexity
    params.filterByConvexity = False  # Disable convexity filter
    ...
    out_image = cv2.drawKeypoints(tuning_image, keypoints, np.array([]), line_color, cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    ...
    # Create a mask to exclude points outside the search window
    mask = np.zeros_like(tuning_image)
    x_min_px, y_min_px, x_max_px, y_max_px = search_window_px
    cv2.rectangle(mask, (x_min_px, y_min_px), (x_max_px, y_max_px), (255, 255, 255), -1)
    out_image = cv2.bitwise_and(out_image, mask)
